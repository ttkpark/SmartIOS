...
void Addstr_destroy(Addstr_Str* str) {
    if(str == 0){ Debug("Deleting NULL pointer\n"); return;}
	if(str->bank > ADDSTR_BANKNUM){ Debug("Deleting Wrong bank%d\n",str->bank); return;}
    //Debug("bank %d free\n",str->bank);
    
    if(str->size>ADDSTR_BUFSIZE)str->size = ADDSTR_BUFSIZE;
	memset(str->str,0,str->size);
	str->size = 0;
}
...
Result Addstr_replace(Addstr_Str *inout,char* Remove,char* Replace)
{
    ...
    // #Added in 2021.07.22 
    //Estimating the buff size...
    
    char* pointer = inout->str;
    int count = 0;
    while(1)
    {
        if((pointer = strstr(pointer,Remove)) == 0)
            break;
        count++;
        pointer += strlen(Remove);
        watchdog();
    }

    if(count == 0)return SUCCEED;//no change is needed.
    
    int estimate_added_size = count*( -strlen(Remove)+strlen(Replace));
    //Debug("replace estimate : %d\n",estimate_added_size);
    
    
    // inout -> in -> result(Replaced) -> out(length optimized) -> inout
    Addstr_Str *result;
	Result res = Addstr_initlen(&result,strlen(inout->str) + estimate_added_size);
    ...
    if(after_len > result->size)
        Debug("Addstr.c:%d : replace Warning!!\n",__LINE__);
	if(after_len+1 > ADDSTR_BUFSIZE)
		Debug("Addstr.c:%d : Memory Overflow!!\n",__LINE__);
