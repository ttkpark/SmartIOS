...
{
  HAL_GPIO_WritePin(GPIOC,GPIO_PIN_9,isonline);
  HAL_GPIO_WritePin(GPIOC,GPIO_PIN_8,!isonline);
}
...
void checkConnectivity(BOOL print)
{
  ...
  if(nic_is_linked && nic_linked_and_ip_address_valid) {
    ...
    */
    ...
    // if(checkUnlinkTimeout(&lastUnlinkedTime,30000))
    // {
    //   Debug("now Resets the Ethernet Device..\n");
    //   Ethernet_Reset();
    //   lastUnlinkedTime = HAL_GetTick();
    // }
    connected = 0;
