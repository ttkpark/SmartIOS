...
{
	uint8_t sendbuf[3];
	uint8_t *getbuf = sendbuf;
	uint8_t rtnData;

	uint8_t sendOp_and_address = op | (address & ADDR_MASK);
	sendbuf[0] = sendOp_and_address;
	...
	if(address & 0x80)
	{
		ENC_SPI_SendBuf(sendbuf,getbuf,3);
		rtnData = getbuf[2];
	}
	// else do the way they was.
	else 
	{
		ENC_SPI_SendBuf(sendbuf,getbuf,2);
		rtnData = getbuf[1];
	}
	CSPASSIVE;
	return rtnData;
...
{
	uint8_t sendbuf[2];
	sendbuf[0] = op | (address & ADDR_MASK);
	sendbuf[1] = data;

	CSACTIVE;
	ENC_SPI_SendBuf(sendbuf,NULL,2);
	CSPASSIVE;
}
//*******************************************************************************************
//
...
void enc28j60_packet_send ( unsigned char *buffer, unsigned short length )
{
	...
	CSACTIVE;
	// issue write command
	ENC_SPI_SendWithoutSelection(ENC28J60_WRITE_BUF_MEM);
	ENC_SPI_SendBuf(buffer,NULL,length);
	// while(length)
	// {
	// 	length--;
	// 	// write data
...
unsigned short enc28j60_packet_receive ( unsigned char *rxtx_buffer, unsigned short max_length )
{
	...
	else
	{
		...
		CSACTIVE;
		// issue read command
		ENC_SPI_SendWithoutSelection(ENC28J60_READ_BUF_MEM);
		ENC_SPI_SendBuf(NULL,rxtx_buffer,rx_status);
		// while(rx_status)
		// {
		// 	rx_status--;
		// 	SPDR = 0x00;
